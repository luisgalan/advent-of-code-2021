...
void solve() {
    ...
    FOR(i, NUM_FOLDS) {
        ...
        FORE(point, points) {
            if (c == 'x') {
                if (point.F >= coord) {
                    next.insert(MP(2 * coord - point.F, point.S));
                } else {
                    next.insert(point);
                }
            } else {
                if (point.S >= coord) {
                    next.insert(MP(point.F, 2 * coord - point.S));
                } else {
                    next.insert(point);
                }
            }
    ...
    FOR(i, 40) {
        FOR(j, 40) {
            cout << ((points.find(MP(j, i)) == points.end()) ? '.' : '#');
        }
        cout << endl;
    }
}
