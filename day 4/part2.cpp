...
bool is_win(vvii &board, int i, int j) {
    bool col = true;
    FOR(di, 5) {
        col &= board[di][j].S;
    }
...
bool check(vvii &board, int value) {
    FOR(i, 5) {
        FOR(j, 5) {
            if (board[i][j].F == value) {
                board[i][j].S = 1;
                if (is_win(board, i, j)) {
                    return 1;
                }
...
void solve() {
    ...
    FOR(i, n) {
        FOR(j, m) {
            if (!already_won[j] && check(boards[j], order[i])) {
                already_won[j] = true;
                last_sum = sum_unmarked(boards[j]);
                last_order = order[i];
            }
        }
    }
