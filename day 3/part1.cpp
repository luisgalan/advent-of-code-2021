...
#define F first
#define S second
#define VEC(x,y) typedef vector<x> v##y;typedef vector<v##y> vv##y;
#define DEF(x,y) typedef x y;typedef vector<x> v##y;typedef vector<v##y> vv##y;
#define COMMA ,
...
void solve() {
    ...
    int epsilon = 0;
    FOR(i, m) {
        gamma |= ((freq[i] > n/2) << (m - i - 1));
        epsilon |= ((freq[i] <= n/2) << (m - i - 1));
    }
    // debug(nums);
    cout << endl << endl;
    cout << bitset<12>(gamma) << endl;
    cout << bitset<12>(epsilon) << endl;
    debug(freq);
    debug(gamma);
    debug(epsilon);
