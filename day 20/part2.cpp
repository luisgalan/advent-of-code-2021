...
void print_img(vector<vector<bool>> img) {
    int n = img.size();
    FOR(i, n) {
        FOR(j, n) {
            cerr << (img[i][j] ? '#' : '.');
...
void solve() {
    vector<bool> filter(512);
    string filter_s; cin >> filter_s;
    FOR(i, 512) {
        filter[i] = filter_s[i] == '#';
    }

    vector<vector<bool>> input(input_size + 2 * pad * depth);
    fill(ALL(input), vector<bool>(input_size + 2 * pad * depth));
    FOR(i, input_size) {
        string s; cin >> s;
        FOR(j, input_size) {
            input[i + pad * depth][j + pad * depth] = s[j] == '#';
        }
    }

    vector<vector<bool>> output(input_size + 2 * pad * depth);
    fill(ALL(output), vector<bool>(input_size + 2 * pad * depth));

    FOR(i, depth) {
        FOR(i, input_size + 2 * pad * depth) {
            FOR(j, input_size + 2 * pad * depth) {
                int idx = 0;
                for (int di = -1; di <= 1; di++) {
                    for (int dj = -1; dj <= 1; dj++) {
                        int ni = clamp(i + di, 0, (int)input.size() - 1);
                        int nj = clamp(j + dj, 0, (int)input.size() - 1);
                        idx = (idx << 1) | input[ni][nj];
                    }
                }
                output[i][j] = filter[idx];
            }
        }
        cerr << endl;
        print_img(output);
        cerr << endl;
        if (i + 1 < depth) swap(input, output);
    }
