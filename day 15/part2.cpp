...
void solve() {
    ...
    FOR(i, 100) {
        ...
        FOR(j, 100) {
            ...
            FOR(ti, 5) {
                FOR(tj, 5) {
                    risks[i + 100 * ti][j + 100 * tj] = (cur_risk + ti + tj - 1) % 9 + 1;
                }
            }
        }
    }
    ...
    while (!pq.empty()) {
        int cur_cost;
        ii cur_pos;
        tie(cur_cost, cur_pos) = pq.top();
        pq.pop();
        debug(cur_pos);
        if (cur_pos == MP(499, 499)) {
            // found goal
            debug(cur_cost);
            break;
        }
        FOR(dir, 4) {
            int ni = cur_pos.F + DY[dir];
            int nj = cur_pos.S + DX[dir];
            if (min(ni, nj) < 0 || max(ni, nj) >= 500) continue;
            int new_cost = cur_cost + risks[ni][nj];
            if (new_cost >= cost[ni][nj]) continue;
            cost[ni][nj] = new_cost;
            pq.push(MP(new_cost, MP(ni, nj)));
        }
    }
}
