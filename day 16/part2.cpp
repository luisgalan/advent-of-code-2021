...
ll apply_op(vll &values, int id) {
    if (id == 0) {
        ...
        ll prod = 1;
        FORE(v, values) prod *= v;
        return prod;
    } else if (id == 2) {
        ll minn = INF;
        FORE(v, values) minn = min(minn, v);
        return minn;
    } else if (id == 3) {
        ll maxx = 0;
        FORE(v, values) maxx = max(maxx, v);
        return maxx;
    } else if (id == 5) {
...
ll parse_lt0(vi &bits, int &idx) {
    ...
    int id = packet_type(&bits[idx]);
    vll values;
    idx += 22;
    int start = idx;
    while (idx - start < num_bits) {
        values.PB(parse_packet(bits, idx));
    }
    return apply_op(values, id);
}
...
ll parse_literal(vi &bits, int &idx) {
    ...
    do {
        idx++;
        value = (value << 4) + parse_number(&bits[idx], 4);
        debug(value);
        idx += 4;
    } while (bits[idx - 5] == 1);
    cerr << "literal with value " << value << endl;
    return value;
}
