...
void accumulate_flashes(int ci, int cj, int (&next)[10][10]) {
    if (next[ci][cj] != 10) return;
    next[ci][cj] = 0;
    ...
    for (int di = -1; di <= 1; di++) {
        for (int dj = -1; dj <= 1; dj++) {
            ...
            int ni = ci + di;
            int nj = cj + dj;
            if (min(ni, nj) < 0 || max(ni, nj) >= 10) continue;
            if (next[ni][nj] > 0 && next[ni][nj] < 10) {
                next[ni][nj]++;
                accumulate_flashes(ni, nj, next);
            }
        }
    }
}

int step(int (&orig)[10][10]) {
    int num_flashes = 0;
    int next[10][10];
    FOR(i, 10) {
        FOR(j, 10) {
    ...
    FOR(i, 10) {
        FOR(j, 10) {
            accumulate_flashes(i, j, next);
        }
    }
    FOR(i, 10) {
        FOR(j, 10) {
            num_flashes += next[i][j] == 0;
            orig[i][j] = next[i][j];
        }
    }
    return num_flashes;
