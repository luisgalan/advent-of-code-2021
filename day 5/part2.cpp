...
void solve() {
    ...
    FOR(i, n) {
        ...
        cin >> _;
        cin >> y2;
        segments[i] = MP(MP(x1, y1), MP(x2, y2));
    }

    vvi grid(SIZE);
    fill(ALL(grid), vi(SIZE));
    FOR(i, n) {
        ii p1 = segments[i].F;
        ii p2 = segments[i].S;
        if (p1.F == p2.F) {
            if (p2.S < p1.S) swap(p1, p2);
            int x = p1.F;
            for (int y = p1.S; y <= p2.S; y++) {
                grid[y][x]++;
            }
        } else if (p1.S == p2.S) {
            if (p2.F < p1.F) swap(p1, p2);
            int y = p1.S;
            for (int x = p1.F; x <= p2.F; x++) {
                grid[y][x]++;
            }
        } else if (p1.F + p1.S == p2.F + p2.S) {
            int c = p1.F + p1.S;
            // Loop through all points such that x + y = c <=> y = c - x
            if (p2.F < p1.F) swap(p1, p2);
            for (int x = p1.F; x <= p2.F; x++) {
                int y = c - x;
                grid[y][x]++;
            ...
            int c = p1.S - p1.F;
            // Loop through all points such that y - x = c <=> y = x + c
            if (p2.F < p1.F) swap(p1, p2);
            for (int x = p1.F; x <= p2.F; x++) {
                int y = x + c;
                grid[y][x]++;
