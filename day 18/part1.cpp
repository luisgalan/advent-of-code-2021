...
bool explode_leftmost(sfn &s, int idx, int depth) {
    ...
    if (
        depth == 4 &&
        s[idx].left
        // s[idx].left && // we're not on a leaf node
        // !s[s[idx].left].left && // left child is a leaf node
        // !s[s[idx].right].left // right child is a leaf node
    ) {
        // cerr << "explode!" << endl;
        explode(s, idx);
        return true;
...
void solve() {
    ...
    sfn final = parse();
    FOR(i, n-1) {
        final = add_sfn(final, parse());
    }
    print_sfn(final);
    cout << endl;

    cout << magnitude(final) << endl;
...
[[[[4,2],2],6],[8,7]]
result of add: [[[[[7,7],[7,7]],[[8,7],[8,7]]],[[[7,0],[7,7]],9]],[[[[4,2],2],6],[8,7]]]


reducing: [[[[0,8],[[8,8],0]],[[10,12],20]],[[[0,4],6],[8,7]]]
explode!
reducing: [[[[0,16],[0,8]],[[10,12],20]],[[[0,4],6],[8,7]]]     <- this split just removes 16?
split!
reducing: [[[[0,0],[0,8]],[[10,12],20]],[[[0,4],6],[8,7]]]
split!
reducing: [[[[0,0],[0,8]],[[[5,5],12],20]],[[[0,4],6],[8,7]]]
