...
int num_paths(int cur_node, vector<bool> visited) {
    if (cur_node == goal) return 1;
    if (!is_big[cur_node] && visited[cur_node]) return 0;
    visited[cur_node] = 1;

    int res = 0;
    FORE(neighbor, edges[cur_node]) {
        res += num_paths(neighbor, visited);
    }
    return res;
}

void solve() {
    vector<pair<string, string>> input(22);
    map<string, int> id;
    int cur_id = 0;
    FOR(i, input.size()) {
        string s;
        ...
        string b = s.substr(f + 1, s.size());
        input[i] = MP(a, b);
        if (id.find(a) == id.end()) id[a] = cur_id++;
        if (id.find(b) == id.end()) id[b] = cur_id++;
    }
    ...
    int ans = num_paths(start, vector<bool>(id.size()));
    debug(ans);
}
